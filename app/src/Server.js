...
const views = {
    ...
    dashboard: path.join(__dirname, '../../', 'public/views/dashboard.html'),
...
const filesPath = [views.landing, views.dashboard, views.newRoom, views.room, views.login, views.whoAreYou, views.activeRooms];
    ...
    */
    ...
    app.get('/', OIDCAuth, (req, res) => {
        ...
        if (!OIDC.enabled && hostCfg.protected) {
            ...
            return htmlInjector.injectHtml(views.dashboard, res);
        }
    });
    ...
    app.get('/dashboard', OIDCAuth, (req, res) => {
        if (!OIDC.enabled && hostCfg.protected) {
            hostCfg.authenticated = false;
            res.redirect('/login');
            return;
    ...
    app.get('/api/dashboard/stats', OIDCAuth, (req, res) => {
        const totalRooms = roomList.size;
        const totalParticipants = Array.from(roomList.values()).reduce((total, room) => total + room.peers.size, 0);

        // Mock storage used for now
        const storageUsed = '2.4 GB';

        res.json({
            activeRooms: totalRooms,
            totalParticipants: totalParticipants,
            totalMeetings: totalRooms, // For now same as active rooms
